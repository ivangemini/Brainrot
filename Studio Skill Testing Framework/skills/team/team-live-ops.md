...
**Expected behavior:**
1. Phase 1: Spawns `live-ops-designer` by activating the relevant role profile; receives season brief with scope, content list, and retention mechanic; presents to user
2. user-decision checkpoint: user approves Phase 1 output before Phase 2 begins
3. Phase 2: Spawns `narrative-director` by activating the relevant role profile; reads the Phase 1 season brief; produces narrative framing document (theme, story hook, lore connections); presents to user
...
6. Phase 6: Spawns `community-manager` by activating the relevant role profile; reads season brief, economy design, and narrative framing; produces communication calendar with draft copy
...
8. user-decision checkpoint: user approves the full season plan
...
- [ ] File writes are delegated to role profiles — orchestrator does not write directly
...
4. user-decision checkpoint presented with resolution options before season plan approval is offered
...
- [ ] user-decision checkpoint is used to surface the violation and offer resolution options (revise economy design, override with documented rationale, cancel)
...
3. Skill exits immediately without spawning any role profiles
...
- [ ] If economy-designer blocks but analytics-engineer succeeds, analytics output is preserved and the block is surfaced via user-decision checkpoint
...
- [ ] `user-decision checkpoint` used at every phase transition — user approves before the next phase begins
- [ ] Phases 3 and 4 are always spawned in parallel, not sequentially
- [ ] File Write Protocol: orchestrator never calls Write/Edit directly — all writes are delegated to role profiles
- [ ] Each output document gets its own "May I write to [path]?" ask from the relevant role profile
- [ ] Ethics review in Phase 7 always references the ethics policy file path explicitly
- [ ] Error recovery: any BLOCKED agent is surfaced immediately with user-decision checkpoint options (skip / retry / stop)
