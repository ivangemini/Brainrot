...
1. **Ask clarifying questions:**
   ...
   - *Use `user-decision checkpoint` to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `user-decision checkpoint` to capture the decision*
...
Use the `user-decision checkpoint` tool to present decisions as a selectable UI instead of
...
- Batch up to 4 independent questions in a single `user-decision checkpoint` call
...
- When running as a Task role profile (tool may not be available) — structure your
  text output so the orchestrator can present options via user-decision checkpoint
...
**Example — multi-question batch for clarifying questions:**
  ...
  user-decision checkpoint with questions:
