...
Spawn `localization-lead` by activating the relevant role profile. Ask them to audit the following for cultural sensitivity across the target locales (read from `assets/data/strings/` and `assets/`):
...
Read `studio/docs/technical-preferences.md` to determine the engine. Then check:
...
Spawn `localization-lead` by activating the relevant role profile with:
