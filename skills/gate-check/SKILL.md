...
- [ ] Technical preferences configured (`studio/docs/technical-preferences.md` populated)
...
Before generating the final verdict, spawn all four directors as **parallel role profiles** by activating the relevant role profile using the parallel gate protocol from `studio/docs/director-gates.md`. Issue all four Task calls simultaneously — do not wait for one before starting the next.

**Spawn in parallel:**

1. **`creative-director`** — gate **CD-PHASE-GATE** (`studio/docs/director-gates.md`)
2. **`technical-director`** — gate **TD-PHASE-GATE** (`studio/docs/director-gates.md`)
3. **`producer`** — gate **PR-PHASE-GATE** (`studio/docs/director-gates.md`)
4. **`art-director`** — gate **AD-PHASE-GATE** (`studio/docs/director-gates.md`)
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using `user-decision checkpoint`.
...
- **No Difficulty Curve doc?** → Create `design/difficulty-curve.md` from the template at `studio/docs/templates/difficulty-curve.md` — or use `/quick-design "difficulty curve"` for a guided session.
- **No player journey map?** → Create `design/player-journey.md` from the template at `studio/docs/templates/player-journey.md` — or author it collaboratively using `/ux-design` Phase 2b.
