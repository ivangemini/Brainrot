...
(parallel) → Integration → Validation → Sign-off. Uses `user-decision checkpoint` at each
phase transition. Delegates all file writes to role profiles. Produces a summary report
...
- [ ] Uses `user-decision checkpoint` at phase transitions for user approval before proceeding
- [ ] Phase 3 is explicitly marked as parallel (gameplay-programmer, ai-programmer, technical-artist, sound-designer)
- [ ] Phase 2 includes spawning the primary engine specialist (read from `studio/docs/technical-preferences.md`)
...
- Engine is configured in `studio/docs/technical-preferences.md` (Engine Specialists section filled)
...
2. Phase 2 — gameplay-programmer + ai-programmer spawned; produce architecture sketch with class structure, interfaces, and file list; then primary engine specialist is spawned to validate idioms; engine specialist output incorporated; `user-decision checkpoint` presented with architecture options before Phase 3 begins
...
4. Phase 4 — integration wires together all Phase 3 outputs; tuning knobs verified as data-driven; `user-decision checkpoint` confirms integration before Phase 5
...
**Assertions:**
- [ ] `user-decision checkpoint` called at each phase gate (at minimum before Phase 3 and before Phase 5)
...
- [ ] All file writes delegated to role profiles (orchestrator never calls Write/Edit directly)
...
### Case 2: Blocked Agent — One role profile returns BLOCKED mid-pipeline
...
4. `user-decision checkpoint` presented with options: (a) Skip ai-programmer and note the gap; (b) Retry with narrower scope; (c) Stop here and run `/architecture-decision` first
...
4. Skill exits without spawning any role profiles
...
**Fixture:**
- `studio/docs/technical-preferences.md` has Engine Specialists section populated (e.g., Primary: godot-specialist)
...
2. Skill reads `studio/docs/technical-preferences.md` Engine Specialists section to identify the primary engine specialist agent type
...
6. `user-decision checkpoint` includes engine specialist's notes alongside the architecture sketch

**Assertions:**
- [ ] Engine specialist agent type is read from `studio/docs/technical-preferences.md` — not hardcoded
...
- [ ] `user-decision checkpoint` used at each phase transition — user approves before pipeline advances
