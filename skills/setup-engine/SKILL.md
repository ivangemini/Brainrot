...
**Question 1 — Prior experience** (ask this first, always, via `user-decision checkpoint`):
...
**Question 2 — Target platform** (ask this second, always, via `user-decision checkpoint` — platform eliminates or heavily weights engines before any other factor):
...
5. Use `user-decision checkpoint` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `user-decision checkpoint` with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
After updating AGENTS.md, create or update `studio/docs/technical-preferences.md` with
...
### Remaining Sections
- **Performance Budgets**: Use `user-decision checkpoint`:
