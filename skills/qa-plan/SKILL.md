...
**Argument:** `$ARGUMENTS` (blank = ask user via user-decision checkpoint)
...
- **No argument** — use `user-decision checkpoint`:
...
**Engine**: [engine name from studio/docs/technical-preferences.md, or "Not configured"]
...
then ask two questions together using `user-decision checkpoint`:
...
- Use `user-decision checkpoint` for scope selection when no argument is provided.
