...
Otherwise, use `user-decision checkpoint` to confirm intent before proceeding:
...
Read `AGENTS.md` and `studio/docs/technical-preferences.md` for the engine and
language in use.
...
Assess which path best fits the hypothesis, then use `user-decision checkpoint` with your
...
Read `studio/docs/templates/prototype-report.md` to get the report structure.
...
- `full` → spawn `creative-director` by activating the relevant role profile using gate **CD-PLAYTEST** if
...
2. **Choose path** — same user-decision checkpoint widget as Phase 3 (HTML / Engine / Paper).
