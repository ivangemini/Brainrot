...
Use the `user-decision checkpoint` tool for implementation choices and next-step decisions.
Follow the **Explain -> Capture** pattern: explain options in conversation, then
call `user-decision checkpoint` with concise labels. Batch up to 4 questions in one call.
