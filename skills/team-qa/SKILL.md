...
- `role profile_type: qa-lead` — Strategy, planning, classification, sign-off
- `role profile_type: qa-tester` — Test case writing and bug report writing
...
Spawn `qa-lead` by activating the relevant role profile to review all in-scope stories and produce a QA strategy.
...
Present the qa-lead's full strategy to the user, then use `user-decision checkpoint`:
...
Spawn `qa-tester` by activating the relevant role profile for each story (run in parallel where possible), providing:
- The story file path
...
Use `user-decision checkpoint` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `user-decision checkpoint` for each:
...
After each FAIL result: use `user-decision checkpoint` to collect the failure description, then spawn `qa-tester` by activating the relevant role profile to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-lead` by activating the relevant role profile to produce the sign-off report using all results from Phases 4–6.
