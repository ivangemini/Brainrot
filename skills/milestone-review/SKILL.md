...
Before generating the Go/No-Go recommendation, spawn `producer` by activating the relevant role profile using gate **PR-MILESTONE** (`studio/docs/director-gates.md`).
...
If OFF TRACK, use `user-decision checkpoint` before generating the recommendation:
...
If AT RISK, use `user-decision checkpoint`:
