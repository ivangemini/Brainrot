...
**If no argument is provided**, do not fail — ask instead. Use `user-decision checkpoint`:
...
> "Player journey map not yet created. Template available at `studio/docs/templates/player-journey.md`. Run `/ux-design` Phase 2b or create it manually to establish player context for this screen."
...
Read `studio/docs/technical-preferences.md` and extract the `## Input & Platform`
...
2. **Questions**: Ask what is needed to draft this section. Use `user-decision checkpoint`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `user-decision checkpoint` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `user-decision checkpoint`:
...
- Use `user-decision checkpoint` to capture the choice:
...
- Use `user-decision checkpoint`: "Want an ASCII wireframe as part of this spec?"
...
Use `user-decision checkpoint` to confirm:
...
Use `user-decision checkpoint` to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use `user-decision checkpoint`:
...
Then use `user-decision checkpoint`:
...
When delegating to another agent via the role-routing mechanism:
