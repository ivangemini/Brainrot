...
Spawn `technical-director` by activating the relevant role profile using gate **TD-MANIFEST** (`studio/docs/director-gates.md`).
...
- **CONCERNS** → surface via `user-decision checkpoint` with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
