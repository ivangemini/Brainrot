...
- [ ] Contains "May I write" language in the File Write Protocol section (delegated to role profiles)
...
- [ ] Uses `user-decision checkpoint` at phase transitions requiring user approval before proceeding
...
**Expected behavior:**
1. Phase 1: Spawns `producer` by activating the relevant role profile; confirms all milestone acceptance criteria met; identifies any deferred scope; produces release authorization; presents to user; user-decision checkpoint: user approves before Phase 2
2. Phase 2: Spawns `release-manager` by activating the relevant role profile; cuts release branch from agreed commit; bumps version numbers; invokes `/release-checklist`; freezes branch; output: branch name and checklist; user-decision checkpoint: user approves before Phase 3
...
5. Phase 5: Spawns `producer` by activating the relevant role profile; collects sign-offs from qa-lead, release-manager, devops-engineer; no open blocking issues; producer declares GO; user-decision checkpoint: user sees GO decision and confirms deployment
...
4. Phase 4 proceeds cautiously or is paused (user-decision checkpoint: continue to Phase 4 or skip to Phase 5 for go/no-go?)
5. Phase 5: Spawns `producer` by activating the relevant role profile; producer receives qa-lead's NOT PASSED verdict; no S1 sign-off available; producer declares NO-GO with rationale: "S1 bug [ID] is open and unresolved. Releasing is not safe."
6. user-decision checkpoint: user is presented with the NO-GO decision and the S1 bug details; options: fix the bug and re-run, defer the release, or override (with documented rationale)
...
- [ ] user-decision checkpoint offers the user resolution options (fix and re-run / defer / override with rationale)
...
4. user-decision checkpoint: options presented — (a) Fix translations and re-run Phase 4, (b) Remove French locale from this release, (c) Ship as-is with a known issues note
...
- [ ] user-decision checkpoint is used to offer the user resolution choices — the skill does not auto-waive
...
3. Confirms with user-decision checkpoint before beginning Phase 1 proper: "Releasing v1.1.0. Is this correct?"
...
2. Uses user-decision checkpoint: "What version number should be released? (e.g., v1.0.0)"
...
- [ ] Inferred version is confirmed with the user via user-decision checkpoint before proceeding (variant A)
- [ ] When no version is discoverable, user-decision checkpoint is used — skill does not guess (variant B)
...
- [ ] `user-decision checkpoint` used at each phase transition gate (post-Phase 1, post-Phase 2, post-Phase 3/4 if issues, post-Phase 5 go/no-go)
...
- [ ] File Write Protocol: orchestrator never calls Write/Edit directly — all writes are delegated to role profiles or sub-skills
...
- The "devops-engineer build fails" path is not separately tested — it would surface as a BLOCKED result in Phase 3 and follow the standard error recovery protocol (surface → assess → user-decision checkpoint options). This is validated structurally by the Static Assertions error recovery check.
...
- The "override NO-GO with documented rationale" path in Case 2 is referenced but not exhaustively tested — it is an escape hatch that the skill must support, and its existence is validated by the user-decision checkpoint options assertion in Case 2.
