...
Use `user-decision checkpoint` to confirm proceeding or reduce scope.
...
Spawn `release-manager` by activating the relevant role profile. Ask them to produce a rollback plan covering:
...
1. Spawn `lead-programmer` by activating the relevant role profile with:
...
3. Spawn `qa-tester` by activating the relevant role profile to verify: does the bug reproduce after the fix?
...
Spawn `qa-lead` by activating the relevant role profile with:
