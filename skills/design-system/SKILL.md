...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `user-decision checkpoint`:
...
- Read `studio/docs/technical-preferences.md` to identify the engine and version
...
Use the template structure from `studio/docs/templates/game-design-document.md`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `user-decision checkpoint` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `user-decision checkpoint` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `user-decision checkpoint`. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use `user-decision checkpoint` with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use `user-decision checkpoint`:
...
spawn `creative-director` by activating the relevant role profile:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents by activating the relevant role profile in parallel:
...
- Surface any disagreements between agents to the user via `user-decision checkpoint`
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents by activating the relevant role profile in parallel:
...
- Present the specialists' proposals to the user for review via `user-decision checkpoint`
...
For required systems: **spawn `art-director` by activating the relevant role profile** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
Before finalizing the GDD, spawn `creative-director` by activating the relevant role profile using gate **CD-GDD-ALIGN** (`studio/docs/director-gates.md`).
...
**When delegating by activating the relevant role profile tool**:
...
- The main session presents the agent's output to the user via `user-decision checkpoint`
- The user decides; the main session writes to file
...
2. **user-decision checkpoint** at every decision point (Explain -> Capture pattern):
