...
and issues READY FOR RELEASE or NEEDS MORE WORK). Uses `user-decision checkpoint` at each
...
2. `user-decision checkpoint` presents performance report; user approves before Phases 2, 3, and 4 begin
...
5. All three parallel phases complete; `user-decision checkpoint` presents results; user approves before Phase 5 begins
6. Phase 5: qa-tester runs edge case tests, soak tests, stress tests, and regression tests; all pass
7. `user-decision checkpoint` presents test results; user approves before Phase 6
...
- [ ] `user-decision checkpoint` appears after Phase 1 output and before Phases 2/3/4 launch
...
2. `user-decision checkpoint` presents the violation; user chooses to proceed with optimization attempt
...
3. `user-decision checkpoint` presents the performance report including the engine-level root cause; user approves before Phase 2
...
7. `user-decision checkpoint` presents the regression with options:
...
- [ ] `user-decision checkpoint` offers options including revert, fix in place, and schedule later
