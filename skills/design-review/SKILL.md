...
| Engine-specific patterns or APIs | Primary engine specialist (from `studio/docs/technical-preferences.md`) |
...
Use `user-decision checkpoint` for ALL closing interactions. Never plain text.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab `user-decision checkpoint` before making any edits — do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker → fix applied) and use `user-decision checkpoint` for a **post-revision closing widget**:
...
When the verdict is APPROVED, use a single `user-decision checkpoint` with `multiSelect: true` to batch the two tracking updates:
...
Use a second `user-decision checkpoint`:
...
Use a third `user-decision checkpoint`:
...
Once the systems-index and review-log widgets are answered, check project state and show one final `user-decision checkpoint`:
