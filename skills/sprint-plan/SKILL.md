...
- If the file doesn't exist and this is a `new` sprint: use `user-decision checkpoint`:
...
3. Ask the user what to change: stories to add, remove, reprioritize, or re-estimate. Use `user-decision checkpoint` to gather changes.
...
Before finalising the sprint plan, spawn `producer` by activating the relevant role profile using gate **PR-SPRINT** (`studio/docs/director-gates.md`).
...
If CONCERNS, use `user-decision checkpoint`:
...
- `full` — run all director gates as spawned role profiles
