...
```
Spawn `creative-director` by activating the relevant role profile using gate **CD-PILLARS** from
...
Spawn `[agent-name]` by activating the relevant role profile:
- Gate: [GATE-ID] (see studio/docs/director-gates.md)
...
Spawn all [N] agents simultaneously by activating the relevant role profile — issue all Task calls before
...
| **CONCERNS [list]** | Issues present but not blocking. | Surface to user via `user-decision checkpoint` — options: `Revise flagged items` / `Accept and proceed` / `Discuss further` |
...
- Game pillars and core fantasy
- Platform and performance constraints (from `studio/docs/technical-preferences.md` if configured)
