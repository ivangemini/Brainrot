...
and world-builder in parallel). Uses `user-decision checkpoint` at each phase transition to
present proposals as selectable options. Produces a narrative summary report and
delivers narrative documents via role profiles that each enforce the "May I write?"
...
- [ ] File writes are delegated to role profiles — orchestrator does not write files directly
...
- [ ] `user-decision checkpoint` is used at phase transitions before proceeding
...
2. `user-decision checkpoint` presents the narrative brief; user approves before Phase 2 begins
...
4. `user-decision checkpoint` presents world foundation and dialogue drafts; user approves before Phase 3 begins
5. Phase 3: level-designer is spawned; produces environmental storytelling layout, trigger placement, and pacing plan
6. `user-decision checkpoint` presents level narrative plan; user approves before Phase 4 begins
...
8. `user-decision checkpoint` presents review results; user approves before Phase 5 begins
...
10. Final summary report is presented; role profile asks "May I write the narrative document to [path]?" before writing
11. Verdict: COMPLETE

**Assertions:**
- [ ] narrative-director is spawned in Phase 1 before any other agents
- [ ] `user-decision checkpoint` appears after Phase 1 output and before Phase 2 launch
- [ ] world-builder and writer Task calls are issued simultaneously in Phase 2 (not sequentially)
- [ ] level-designer is not launched until Phase 2 `user-decision checkpoint` is approved
...
6. `user-decision checkpoint` presents options:
...
- [ ] `user-decision checkpoint` presents at least 3 options including "stop and resolve first"
...
- [ ] No `user-decision checkpoint` is used — output is direct guidance
...
5. `user-decision checkpoint` presents options:
...
- [ ] `user-decision checkpoint` offers the option to fix now vs. flag and proceed
...
- [ ] `user-decision checkpoint` offers at least one option to resolve the missing prerequisite
...
- [ ] `user-decision checkpoint` is used after every phase output before the next phase launches
...
- [ ] No files are written by the orchestrator directly — all writes are delegated to role profiles
- [ ] Each role profile enforces the "May I write to [path]?" protocol before any write
...
- The "Retry with narrower scope" and "Skip this agent" resolution paths from the Error
  Recovery Protocol are not separately tested — they follow the same `user-decision checkpoint`
