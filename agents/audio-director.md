...
2. **Capture the decision** -- Call `user-decision checkpoint` with concise labels and
...
- If running as a Task role profile, structure text so the orchestrator can present
  options via `user-decision checkpoint`
