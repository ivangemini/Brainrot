...
3. `studio/docs/technical-preferences.md` — naming conventions, performance budgets,
...
Apply gate **TD-ARCHITECTURE** (`studio/docs/director-gates.md`) as a self-review. Check all four criteria from that gate definition against the completed document.
...
**Step 2 — Spawn `lead-programmer` by activating the relevant role profile using gate LP-FEASIBILITY (`studio/docs/director-gates.md`):**
...
Use `user-decision checkpoint` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `user-decision checkpoint`:
