...
`user-decision checkpoint` at each step transition. Verdict is COMPLETE when the audio
...
- [ ] `user-decision checkpoint` is used at step transitions before proceeding
...
- Engine is configured in `studio/docs/technical-preferences.md`
...
3. `user-decision checkpoint` presents audio direction; user approves before Step 2 begins
...
5. `user-decision checkpoint` presents SFX spec and accessibility requirements; user approves before Step 3 begins
...
7. `user-decision checkpoint` presents technical plan; user approves before Step 4 begins
...
- [ ] `user-decision checkpoint` appears after Step 1 output and before Step 2 launch
...
- [ ] gameplay-programmer is not launched until Step 3 `user-decision checkpoint` is approved
...
3. Orchestrator surfaces the concern immediately in conversation before presenting `user-decision checkpoint`
4. `user-decision checkpoint` presents the accessibility concern as a BLOCKING issue with options:
...
- [ ] `user-decision checkpoint` surfaces the gap before Step 3 is launched
...
**Fixture:**
- Engine is NOT configured in `studio/docs/technical-preferences.md` (shows `[TO BE CONFIGURED]`)
- GDD for the target feature exists
...
**Expected behavior:**
1. Context gathering: orchestrator reads `studio/docs/technical-preferences.md` and detects no engine is configured
...
- [ ] `user-decision checkpoint` is used after every step output before the next step launches
