...
See `studio/docs/director-gates.md` for the full check pattern and mode definitions.

---

## 1. Parse Arguments

**Scope:** `$ARGUMENTS[0]` (blank = ask user via user-decision checkpoint)
...
If no argument is given, use `user-decision checkpoint`:
...
Spawn `qa-lead` by activating the relevant role profile using gate **QL-STORY-READY** (`studio/docs/director-gates.md`).
...
- **GAPS [list]** → surface the specific gaps to the user via `user-decision checkpoint`:
