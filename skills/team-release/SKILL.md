...
3. If no version is discoverable: use `user-decision checkpoint` to ask "What version number should be released? (e.g., v1.0.0)" and wait for user input before proceeding. Do NOT default to a hardcoded version string.
...
- `role profile_type: release-manager` — Release branch, versioning, changelog, deployment
- `role profile_type: qa-lead` — Test sign-off, regression suite, release quality gate
- `role profile_type: devops-engineer` — Build pipeline, artifacts, deployment automation
- `role profile_type: security-engineer` — Security audit for online/multiplayer/data features
- `role profile_type: analytics-engineer` — Telemetry event verification and dashboard readiness
- `role profile_type: community-manager` — Patch notes and launch communication
- `role profile_type: producer` — Go/no-go decision, stakeholder communication
- `role profile_type: network-programmer` — Netcode stability sign-off (invoke if game has multiplayer)
