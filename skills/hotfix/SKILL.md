...
Confirm with `user-decision checkpoint`:
...
If the check passes, use `user-decision checkpoint` before creating the branch:
...
Use the role-routing mechanism to request sign-off in parallel:

- `role profile_type: lead-programmer` — Review the fix for correctness and side effects
- `role profile_type: qa-tester` — Run targeted regression tests on the affected system
- `role profile_type: producer` — Approve deployment timing and communication plan
...
After approvals, determine the QA scope required before deploying the hotfix. Spawn `qa-lead` by activating the relevant role profile with:
