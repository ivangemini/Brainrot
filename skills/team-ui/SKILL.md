...
- **engine UI specialist** — Validates UI implementation patterns against engine-specific best practices (read from `studio/docs/technical-preferences.md` Engine Specialists → UI Specialist)
...
- `role profile_type: ux-designer` — User flows, wireframes, accessibility, input handling
- `role profile_type: ui-programmer` — UI framework, screens, widgets, data binding
- `role profile_type: art-director` — Visual style, layout polish, art bible consistency
- `role profile_type: [UI engine specialist]` — Engine-specific UI pattern validation (e.g., unity-ui-specialist, ue-umg-specialist, godot-specialist)
- `role profile_type: accessibility-specialist` — Accessibility compliance audit
...
Then use `user-decision checkpoint` with options:
...
Before implementation begins, spawn the **engine UI specialist** (from `studio/docs/technical-preferences.md` Engine Specialists → UI Specialist) to review the UX spec and visual design spec for engine-specific implementation guidance:
