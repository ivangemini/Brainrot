...
After completing the engine audit above, spawn the **primary engine specialist** by activating the relevant role profile for a domain-expert second opinion:
...
- Spawn `role profile_type: [primary specialist]` with: all ADRs that contain engine-specific decisions or `Post-Cutoff APIs Used` fields, the engine reference docs, and the Phase 5 audit findings. Ask them to:
...
If [B]: display the complete proposed systems-index section, then re-ask with `user-decision checkpoint`.
...
Use `user-decision checkpoint` for the write approval:
...
Then close with `user-decision checkpoint` tailored to the pre-gate checklist state:
...
If any activated role returns BLOCKED, errors, or fails to complete:
...
3. **Offer options** via user-decision checkpoint with three choices:
...
5. **Use `user-decision checkpoint` for write approvals** — plain text "May I?" is not
