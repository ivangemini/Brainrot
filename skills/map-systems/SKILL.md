...
- Use `user-decision checkpoint` to ask:
...
Then use `user-decision checkpoint` to capture feedback:
...
Use `user-decision checkpoint` to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` by activating the relevant role profile using gate TD-SYSTEM-BOUNDARY (`studio/docs/director-gates.md`) before proceeding to priority assignment.**
...
Use `user-decision checkpoint` to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` by activating the relevant role profile using gate PR-SCOPE (`studio/docs/director-gates.md`) before writing the index.**
...
Using the template at `studio/docs/templates/systems-index.md`, populate the
...
**After the systems index is written, spawn `creative-director` by activating the relevant role profile using gate CD-SYSTEMS (`studio/docs/director-gates.md`).**
...
Use `user-decision checkpoint` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `/design-system` completes, use `user-decision checkpoint`:
...
After the systems index is created (or after designing some systems), present next actions using `user-decision checkpoint`:
