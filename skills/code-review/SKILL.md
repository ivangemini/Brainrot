...
Read `studio/docs/technical-preferences.md`, section `## Engine Specialists`. Note:
...
Spawn all applicable specialists simultaneously by activating the relevant role profile — do not wait for one before starting the next.
...
For Logic and Integration stories, also spawn `qa-tester` by activating the relevant role profile in parallel with the engine specialists. Pass:
