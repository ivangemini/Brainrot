...
Check:
- **Engine configured?** Read `studio/docs/technical-preferences.md`. If the Engine field contains `[TO BE CONFIGURED]`, the engine is not set.
...
This is the first thing the user sees. Use `user-decision checkpoint` with these exact options so the user can click rather than type:

- **Prompt**: "Welcome to Codex Game Studio Framework! Before I suggest anything, I'd like to understand where you're starting from. Where are you at with your game idea right now?"
- **Options**:
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not user-decision checkpoint (it's an open response).
2. Acknowledge the concept, then use `user-decision checkpoint` to offer two paths:
...
**If it does not exist**: Use `user-decision checkpoint`:
...
After presenting the recommended path, use `user-decision checkpoint` to ask the user which step they'd like to take first. Never auto-run the next skill.
