...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `user-decision checkpoint`** at key decision points throughout brainstorming:
   ...
   `user-decision checkpoint` to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an user-decision checkpoint with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `user-decision checkpoint` for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab `user-decision checkpoint` with these exact tab labels:
...
Present all three. Then use `user-decision checkpoint` to capture the selection.
...
Ask these as `user-decision checkpoint` calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `user-decision checkpoint`:
...
If the user selects B, C, or D, make the revision, then use `user-decision checkpoint` again:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` by activating the relevant role profile in parallel before moving to Phase 5. Issue both Task calls simultaneously — do not wait for one before starting the other.**

- **`creative-director`** — gate **CD-PILLARS** (`studio/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`studio/docs/director-gates.md`)
...
Collect both verdicts, then present them together using a two-tab `user-decision checkpoint`:
...
- **Target platform**: Use `user-decision checkpoint` — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `user-decision checkpoint` — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, spawn `technical-director` by activating the relevant role profile using gate TD-FEASIBILITY (`studio/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` by activating the relevant role profile using gate PR-SCOPE (`studio/docs/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `studio/docs/templates/game-concept.md`. Fill in ALL sections from the
...
After revising, show the updated section as a diff or clear before/after, then use `user-decision checkpoint` — "Ready to write the updated concept document?"
Options: `[A] Yes — write it` / `[B] Revise another section`
Repeat until the user selects [A].

If yes, generate the document using the template at `studio/docs/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
