...
- [ ] Uses `user-decision checkpoint` at phase transitions to capture user approval before proceeding
...
2. Phase 2: Spawns `qa-lead` by activating the relevant role profile; produces strategy table classifying all 4 stories; no blockers flagged; presents to user; user-decision checkpoint: user selects "Looks good — proceed to test plan"
...
4. Phase 4: Spawns `qa-lead` by activating the relevant role profile; reviews `tests/smoke/`; returns PASS; reports "Smoke check passed. Proceeding to test case writing."
5. Phase 5: Spawns `qa-tester` by activating the relevant role profile for each Visual/Feel and Integration story (2–3 stories); run in parallel; presents test cases grouped by story; user-decision checkpoint per group; user approves
6. Phase 6: Walks through each approved story; user marks all as PASS; result summary: "Stories PASS: 4, FAIL: 0, BLOCKED: 0"
7. Phase 7: Spawns `qa-lead` by activating the relevant role profile to produce sign-off report; report shows all stories PASS; no bugs filed; Verdict: APPROVED; asks "May I write this QA sign-off report to `production/qa/qa-signoff-sprint-03-[date].md`?"; writes after approval
...
2. Phase 4: Spawns `qa-lead` by activating the relevant role profile; smoke check returns FAIL; two specific failures are identified
...
2. Phase 6: User marks Visual/Feel story as FAIL; user-decision checkpoint collects failure description: "Animation plays at 2x speed — jitter visible on every loop"
3. Phase 6: Spawns `qa-tester` by activating the relevant role profile to write a formal bug report; bug report written to `production/qa/bugs/BUG-001-animation-speed-jitter.md` (or next increment if bugs exist); report includes severity field
...
**Assertions:**
- [ ] FAIL result in Phase 6 triggers user-decision checkpoint to collect the failure description before the bug report is written
- [ ] `qa-tester` is spawned by activating the relevant role profile to write the bug report — orchestrator does not write it directly
...
2. Cannot infer sprint; uses user-decision checkpoint: "Which sprint or feature should QA cover?" with options to type a sprint identifier or cancel
...
- [ ] When both state files are absent, skill uses user-decision checkpoint rather than guessing (variant B)
...
- [ ] `user-decision checkpoint` used at Phase 2 (strategy review), Phase 5 (test case approval per group), and Phase 6 (per-story manual QA result)
...
- [ ] Bug reports are always written by `qa-tester` by activating the relevant role profile — orchestrator does not write directly
- [ ] Phase 5 qa-tester tasks for independent stories are issued in parallel where possible
- [ ] Error recovery: any BLOCKED agent is surfaced immediately with user-decision checkpoint options
