...
in this skill — apply the check pattern from `studio/docs/director-gates.md`
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` by activating the relevant role profile using gate **QL-STORY-READY** (`studio/docs/director-gates.md`).
...
Use `user-decision checkpoint` to close with context-aware next steps:
