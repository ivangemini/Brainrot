...
Templates are in `studio/docs/templates/`:
...
Also in `studio/docs/templates/collaborative-protocols/` (used by agents, not typically edited directly):
...
2. **Set up the engine** — Run `/setup-engine` (uses the brainstorm recommendation)
   ...
   - Creates `studio/docs/technical-preferences.md` with naming conventions,
