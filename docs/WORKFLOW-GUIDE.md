...
> defined in `studio/docs/workflow-catalog.yaml` and read by `/help`.
...
```
=== Codex Game Studio Framework -- Session Context ===
Branch: main
Recent commits:
...
If you see this, hooks are working. If not, check `AGENTS.md` to
...
studio/docs/templates/post-mortem.md
```
...
Full gate definitions and check pattern: `studio/docs/director-gates.md`
...
### The user-decision checkpoint Tool

Agents use the `user-decision checkpoint` tool for structured option presentation.
...
| `validate-skill-change.sh` | Skill file written | Advises running `/skill-test` after `skills/` changes |
