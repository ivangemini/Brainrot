...
- `role profile_type: audio-director` — Sonic identity, emotional tone, audio palette
- `role profile_type: sound-designer` — SFX specifications, audio events, mixing groups
- `role profile_type: technical-artist` — Audio middleware, bus structure, memory budgets
- `role profile_type: [primary engine specialist]` — Validate audio integration patterns for the engine
- `role profile_type: gameplay-programmer` — Audio manager, gameplay triggers, adaptive music
...
Spawn the **primary engine specialist** in parallel (from `studio/docs/technical-preferences.md` Engine Specialists) to validate the integration approach:
...
5. **Save to** `design/audio/audio-[feature].md`.
   ...
   Note: If `design/audio/` does not exist, the role profile writing the document should create it (the directory will be created automatically when the file is written).
