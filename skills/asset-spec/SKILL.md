...
Present the full proposed inventory to the user in conversation. Then use `user-decision checkpoint`:
...
- **Technical preferences**: Read `studio/docs/technical-preferences.md` — extract performance budgets and naming conventions.
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use `user-decision checkpoint`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use `user-decision checkpoint`:
...
Present the full identified list to the user. Use `user-decision checkpoint`:
...
**`art-director`** by activating the relevant role profile:
...
**`technical-artist`** by activating the relevant role profile:
...
After presenting all specs, use `user-decision checkpoint`:
...
If [B]: ask which asset and what to change. Revise inline and re-present. Do NOT re-activate role profiles for minor text revisions — only re-spawn if the visual direction itself needs to change.
...
If any activated role returns BLOCKED or cannot complete:
