...
Uses `user-decision checkpoint` at each phase transition. Delegates all file writes to
role profiles and sub-skills (`/ux-design`, `ui-programmer`). Produces a summary report
...
- [ ] Contains "May I write" or "File Write Protocol" — writes delegated to role profiles and sub-skills, orchestrator does not write files directly
...
- Engine UI specialist configured in `studio/docs/technical-preferences.md`
...
2. Phase 1b — `/ux-design inventory-screen` invoked (or ux-designer spawned directly); produces `design/ux/inventory-screen.md` using `ux-spec.md` template; `user-decision checkpoint` confirms spec before review
3. Phase 1c — `/ux-review design/ux/inventory-screen.md` invoked; returns APPROVED; gate passed, proceed to Phase 2
4. Phase 2 — art-director spawned; reviews full UX spec (not only wireframes); applies visual treatment; verifies color contrast; produces visual design spec with asset manifest; `user-decision checkpoint` confirms before Phase 3
...
- [ ] All file writes delegated to role profiles and sub-skills
...
4. `user-decision checkpoint` presented with the specific flagged concerns and options:
...
- [ ] `user-decision checkpoint` presents the specific flagged concerns before offering options
...
4. Skill exits without spawning any role profiles or reading any project files
...
5. `user-decision checkpoint` presents all three review results before Phase 5 begins
...
- [ ] `user-decision checkpoint` shows all three review streams' results together before Phase 5 approval
...
3. `user-decision checkpoint` presented with options:
...
- [ ] `user-decision checkpoint` offers a "create pattern library first" option (referencing `/ux-design patterns`)
...
- [ ] All file writes delegated to role profiles and sub-skills — orchestrator does not call Write or Edit directly
