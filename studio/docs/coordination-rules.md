...
Skills with `execution-profile: lightweight-review`: `/help`, `/sprint-status`, `/story-readiness`, `/scope-check`,
`/project-stage-detect`, `/changelog`, `/patch-notes`, `/onboard`

Skills with `execution-profile: deep-review`: `/review-all-gdds`, `/architecture-review`, `/gate-check`
...
**When to spawn in parallel**: If two role profiles' inputs are independent (neither
...
- One session's output is required as input for another (use sequential role profiles)
- The task fits in a single session's context (use role profiles instead)
