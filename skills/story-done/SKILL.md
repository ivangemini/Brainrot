...
3. If multiple in-progress stories are found, use `user-decision checkpoint`:
...
### Manual verification with confirmation (use `user-decision checkpoint`)
...
Batch up to 4 manual verification questions into a single `user-decision checkpoint` call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Manual confirmation**: if the criterion was verified via `user-decision checkpoint`
...
After completing the deviation checks in Phase 4, spawn `qa-lead` by activating the relevant role profile using gate **QL-TEST-COVERAGE** (`studio/docs/director-gates.md`).
...
- `lean` → use `user-decision checkpoint` before proceeding:
...
Spawn `lead-programmer` by activating the relevant role profile using gate **LP-CODE-REVIEW** (`studio/docs/director-gates.md`).

Pass: implementation file paths, story file path, relevant GDD section, governing ADR.

Present the verdict to the user. If CONCERNS, surface them via `user-decision checkpoint`:
...
Use `user-decision checkpoint` before writing anything:
...
- Use `user-decision checkpoint` for the code review prompt and for batching manual
