...
- `studio/docs/technical-preferences.md` — engine configured?
...
If the project appears fresh (no artifacts at all), use `user-decision checkpoint`:
...
Read `studio/docs/technical-preferences.md`. Check each field for `[TO BE CONFIGURED]`:
...
and offer to handle it immediately using `user-decision checkpoint`. Choose the first
branch that applies:
