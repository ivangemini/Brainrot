...
a **confirm/adjust** prompt using `user-decision checkpoint` — not open-ended questions.
...
decisions remain, present each one as a separate `user-decision checkpoint` with the proposed
...
5.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** by activating the relevant role profile to validate the drafted ADR:
   - Read `studio/docs/technical-preferences.md` `Engine Specialists` section to get the primary specialist
   - If no engine is configured (`[TO BE CONFIGURED]`), skip this step
   - Spawn `role profile_type: [primary specialist]` with: the ADR's Engine Compatibility section, Decision section, Key Interfaces, and the engine reference docs path. Ask them to:
...
5.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` by activating the relevant role profile using gate **TD-ADR** (`studio/docs/director-gates.md`):
...
5. **Write approval** — Use `user-decision checkpoint`:
...
Ask using `user-decision checkpoint`:
...
After the ADR is written (and registry optionally updated), close with `user-decision checkpoint`.
