...
against sprint stories, and uses `user-decision checkpoint` to batch-verify manual smoke
...
5. Uses `user-decision checkpoint` for Batch 1 (core stability) and Batch 2 (sprint mechanics)
...
- [ ] `user-decision checkpoint` is used for manual smoke check batches
...
### Case 3: Manual Confirmation — user-decision checkpoint used, PASS WITH WARNINGS
...
3. `user-decision checkpoint` is used for Batch 1 and Batch 2 — developer confirms all PASS
...
**Assertions:**
- [ ] `user-decision checkpoint` is used for manual smoke check batches (not inline text prompts)
...
- [ ] Uses `user-decision checkpoint` for all manual smoke check batches (Batch 1, Batch 2, Batch 3)
...
- The `--platform` argument adds platform-specific user-decision checkpoint batches and a
