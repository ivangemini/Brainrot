...
document saved to `design/levels/[level-name].md`. Uses `user-decision checkpoint` at each
step transition. Delegates all file writes to role profiles. Produces a summary report
...
- [ ] Contains "May I write" or "File Write Protocol" — writes delegated to role profiles, orchestrator does not write files directly
...
- [ ] Uses `user-decision checkpoint` at step transitions for user approval before proceeding
...
2. Step 1 — narrative-director spawned: defines narrative purpose, key characters, dialogue triggers, emotional arc; world-builder spawned: provides lore context, environmental storytelling opportunities, world rules; `user-decision checkpoint` confirms Step 1 outputs before Step 2
3. Step 2 — level-designer spawned: designs spatial layout (critical path, optional paths, secrets), pacing curve, encounters, puzzles, entry/exit points and connections to adjacent areas; `user-decision checkpoint` confirms layout before Step 3
4. Step 3 — systems-designer spawned: specifies enemy compositions, loot tables, difficulty balance, area-specific mechanics, resource distribution; `user-decision checkpoint` confirms systems before Step 4
5. Step 4 — art-director and accessibility-specialist spawned in parallel; art-director: visual theme, color palette, lighting, asset list, VFX needs; accessibility-specialist: navigation clarity, colorblind safety, cognitive load check — each concern rated BLOCKING / RECOMMENDED / NICE TO HAVE; `user-decision checkpoint` presents both outputs before Step 5
...
7. Orchestrator compiles all team outputs into level design document format; role profile asked "May I write to `design/levels/forest-dungeon.md`?"; file saved
...
- [ ] `user-decision checkpoint` called at each step gate (minimum: after Step 1, Step 2, Step 3, Step 4)
- [ ] Step 4 agents (art-director, accessibility-specialist) launched simultaneously
- [ ] All file writes delegated to role profiles — orchestrator does not write directly
...
4. `user-decision checkpoint` presented with options:
...
- [ ] `user-decision checkpoint` offers at minimum three options (skip / retry / stop)
...
4. Skill exits without reading any project files or spawning any role profiles

**Assertions:**
- [ ] Skill does NOT spawn any role profiles when no argument is given
...
4. Skill presents both Step 4 results via `user-decision checkpoint` — BLOCKING concern highlighted prominently
5. `user-decision checkpoint` offers:
...
- [ ] `user-decision checkpoint` presents the specific concern text (not just "accessibility issue found")
...
2. Orchestrator (or level-designer role profile) checks `design/levels/` for `crystal-caves.md`; file not found
...
- [ ] `user-decision checkpoint` offers a "design crystal caves first" option referencing `/team-level`
...
- [ ] `user-decision checkpoint` used at each step transition — user approves before pipeline advances
- [ ] All file writes delegated to role profiles by activating the relevant role profile — orchestrator does not call Write or Edit directly
