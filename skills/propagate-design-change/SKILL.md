...
Spawn `technical-director` by activating the relevant role profile using gate **TD-CHANGE-IMPACT** (`studio/docs/director-gates.md`).
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged; use `user-decision checkpoint` with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`
